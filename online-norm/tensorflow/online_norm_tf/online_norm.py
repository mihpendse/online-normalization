...
class ControlNorm(Layer):
    ...
    def __init__(self, alpha_fwd=0.999, alpha_bkw=0.99,
        ...
        super(ControlNorm, self).__init__(trainable=trainable,
                                         name=name, **kwargs)
    ...
    def control_normalization(self, inputs):
        ...
        def backward(deltas):
            ...
            with tf.control_dependencies([deltas, self.outputs, self.s]):
                ...
                grad_delta = (
                    delta_temp_scaled -
                    (1 - alpha_bkw) * tf.reshape(self.u_ctrl, self.broadcast_shape)
                )
...
class OnlineNorm(Layer):
    ...
    def __init__(self, alpha_fwd=0.999, alpha_bkw=0.99, layer_scaling=True,
        ...
        self.control_normalization = ControlNorm(
            alpha_fwd, alpha_bkw, axis, epsilon,
            stream_mu_initializer, stream_var_initializer,
            u_ctrl_initializer, v_ctrl_initializer, trainable
        )
